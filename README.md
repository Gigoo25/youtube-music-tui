...
- **Custom palette** from an optional `colors.json` (see [Custom colors](#custom-colors))
...
An optional `colors.json` next to `config.json` (`~/.config/ytmusic/colors.json`)
becomes a `desktop` theme that is selected at every start, over the theme saved in
`config.json`. The app never writes it, so it can be generated by a dotfiles setup
from the desktop palette. `T` still cycles themes for the session.
...
Every field is optional and must be `#rrggbb`; a missing or malformed one falls
back to the default theme's value. `primary` is the main accent (headers,
selection, borders, progress) and `background` is the text color drawn on top of
it.
