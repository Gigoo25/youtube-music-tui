...
func New(p *player.Player, cfg *config.Config) *model {
	...
	// A colors.json next to config.json (e.g. generated by a dotfiles setup
	// from the desktop palette) becomes the "desktop" theme and is selected at
	// startup, so the managed palette wins over the saved choice. Cycling with
	// the theme key still works for the session.
	fromFile := false
	if t, ok := loadThemeFile(config.ColorsPath()); ok {
		themes = withDesktopTheme(themes, t)
		fromFile = true
	}

	// Otherwise restore the saved color theme (default to the first if
	// unset/unknown).
	themeIdx := 0
	if i := themeIndex(cfg.Theme); !fromFile && i >= 0 {
		themeIdx = i
	}
